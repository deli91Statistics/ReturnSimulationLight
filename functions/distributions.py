...
class SkewedTStudent(ErrorDistribution):
    """
    This is generated code, need to be checked!
    ...
    def __init__(self, 
        ...
        super().__init__(random_state=random_state)
        self._name = 'Skewed-T-Student'
        self.df = df
        self.skewness = skewness
        self.param_count = 2
        self.param_names = ['df', 'skewness']
        self.param_bounds = [(1, np.inf), (1, np.inf)]

    @staticmethod
    def log_density(data: Union[float, np.array], 
                    sigma2: Union[float, np.array],
                    df: int,
                    skewness: int) -> Union[float, np.array]:
        
        # If lambda > 0 skewness is to the right (positive skew) else to the left (negative skew)
        if skewness > 0:
            z = data / np.sqrt(sigma2)
        else:
            z = - data / np.sqrt(sigma2)
        
        aux = 1 + skewness * z
        indicator = np.where(aux > 0, 1, 0)
        
        lls = gammaln((df + 1) / 2) - gammaln(df / 2) - 0.5 * np.log(np.pi * df)
        lls -= 0.5 * np.log(sigma2)
        lls += np.log(2) + np.log(indicator)
        lls -= ((df + 1) / 2) * np.log(1 + (z ** 2.0) / df)
    ...
    def generate_std_innovations(self, size: int) -> Union[float, np.array]:
        ...
        # Skewed T-Student is more involved. One way to generate it is by mixing Gaussian and 
        # Chi-Square random variables, then adjusting for skewness.
        # This is a simple method for illustration, and there might be more efficient methods in practice.
        
        u = self.random_state.uniform(size=size)
        g = self.random_state.normal(size=size)
        
        chi2_val = self.random_state.chisquare(df=self.df, size=size)
        z = g / np.sqrt(chi2_val / self.df)
        
        t_val = np.where(u < 0.5, z, -z)  # Flip sign with probability 0.5 for symmetry
        skewed_t = t_val + self.skewness * np.abs(t_val)
